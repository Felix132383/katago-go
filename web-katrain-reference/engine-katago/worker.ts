...
/**
 * 加载并安装模型。
 * 优先使用主线程传入的 modelData（ArrayBuffer/Uint8Array，避免 worker 内 fetch 卡住）；
 * 未传入时回退为从 modelUrl fetch。
 */
async function ensureModel(
  ...
  if (modelData && modelData.byteLength) {
    const buf = modelData instanceof Uint8Array ? modelData : new Uint8Array(modelData);
    data = maybeUngzip(buf);
  } else {
    const res = await fetch(modelUrl);
    if (!res.ok) throw new Error(`Failed to fetch model: ${res.status} ${res.statusText}`);
    data = maybeUngzip(new Uint8Array(await res.arrayBuffer()));
  }
...
async function handleMessage(msg: KataGoWorkerRequest): Promise<void> {
  if (msg.type === 'katago:init') {
    await ensureModel(msg.modelUrl, msg.backend, msg.modelData);
    post({

...
function collectGroupAndLiberties(stones, start, color, maxLibertiesToCount) {
    bfsStamp++;
...
export function playMove(pos, move, player, captureStack) {
...
export function undoMove(pos, move, player, snapshot, captureStack) {
...
export function computeLibertyMapInto(stones, out) {
...
export function updateLibertyMapForSeeds(stones, seeds, seedCount, out) {
...
export function computeLibertyMap(stones) {
...
function isAdjacentToColor(stones, pos, color) {
    const nStart = NEIGHBOR_START[pos];
    const nCount = NEIGHBOR_COUNT[pos];
    for(let i = 0; i < nCount; i++){
        const n = NEIGHBORS[nStart + i];
        if (stones[n] === color) return true;
    }
    return false;
}
function isAdjacentToPlaGroup(stones, pos, plaColor, plaGroup) {
    const nStart = NEIGHBOR_START[pos];
    const nCount = NEIGHBOR_COUNT[pos];
    for(let i = 0; i < nCount; i++){
        const n = NEIGHBORS[nStart + i];
        if (stones[n] !== plaColor) continue;
...
function calculateAreaForPla(args) {
    ...
    const buildRegion = (initialPos, regionIdx)=>{
        ...
        while(qh !== qt){
            ...
            if (hasVital && (isMultiStoneSuicideLegal || stones[pos] === EMPTY)) {
            ...
            if (NUM_INTERNAL_SPACES_MAX2[regionIdx] < 2 && !isAdjacentToColor(stones, pos, plaColor)) {
                NUM_INTERNAL_SPACES_MAX2[regionIdx] = NUM_INTERNAL_SPACES_MAX2[regionIdx] + 1;
            }
            if (stones[pos] === oppColor) CONTAINS_OPP[regionIdx] = 1;
    ...
    for(let p = 0; p < BOARD_AREA; p++){
        ...
        let initialVLen = 0;
        {
            ...
            for(let i = 0; i < nCount; i++){
                ...
                if (stones[adj] !== plaColor) continue;
    ...
    for(let g = 0; g < numGroups; g++){
        if (GROUP_COLOR_BY_GROUP[g] === plaColor) {
    ...
    for(let i = 0; i < numRegions; i++){
        ...
        for(let j = 0; j < vLen; j++){
            ...
            VITAL_COUNT_BY_GROUP[g] = VITAL_COUNT_BY_GROUP[g] + 1;
        }
    }
    while(true){
            ...
            for(let t = 0; t < len; t++){
                ...
                for(let k = 0; k < nCount; k++){
                    ...
                    for(let u = 0; u < vl; u++){
                        ...
                        VITAL_COUNT_BY_GROUP[gg] = VITAL_COUNT_BY_GROUP[gg] - 1;
                    }
                }
            }
        }
    ...
    for(let i = 0; i < numRegions; i++){
        ...
        if (shouldMark) {
            ...
            if (shouldMarkIfEmpty) {
                let cur = headPos;
                do {
                    if (result[cur] === EMPTY) result[cur] = plaColor;
                    cur = NEXT_EMPTY_OR_OPP[cur];
                }while (cur !== headPos)
            }
        }
...
export function computeAreaMapV7KataGo(stones, isMultiStoneSuicideLegal = false) {
    return computeAreaMapV7KataGoInto(stones, new Uint8Array(BOARD_AREA), isMultiStoneSuicideLegal);
}
export function computeAreaMapV7KataGoInto(stones, out, isMultiStoneSuicideLegal = false) {
...
function getNumImmediateLiberties(stones, pos) {
    let num = 0;
    const nStart = NEIGHBOR_START[pos];
    const nCount = NEIGHBOR_COUNT[pos];
    for(let i = 0; i < nCount; i++){
        const n = NEIGHBORS[nStart + i];
        if (stones[n] === EMPTY) num++;
    }
    return num;
}
function findLibertiesIntoBuf(stones, start, color, buf, bufIdx, max) {
    bfsStamp++;
    const stamp = bfsStamp;
    let stackLen = 0;
    let groupLen = 0;
...
function getNumLibertiesCapped(stones, loc, cap) {
    const c = stones[loc];
    if (c === EMPTY) return 0;
    return collectGroupAndLiberties(stones, loc, c, cap).liberties;
}
function wouldBeKoCapture(stones, loc, pla) {
    if (stones[loc] !== EMPTY) return false;
...
function hasLibertyGainingCaptures(stones, loc) {
    const pla = stones[loc];
    ...
    for(let i = 0; i < g.groupLen; i++){
        ...
        for(let j = 0; j < nCount; j++){
            ...
            if (stones[adj] !== opp) continue;
            if (getNumLibertiesCapped(stones, adj, 2) === 1) return true;
        }
    }
    return false;
}
function countHeuristicConnectionLibertiesX2(stones, loc, pla) {
    let numLibsX2 = 0;
    const nStart = NEIGHBOR_START[loc];
    const nCount = NEIGHBOR_COUNT[loc];
    for(let i = 0; i < nCount; i++){
        const adj = NEIGHBORS[nStart + i];
        if (stones[adj] !== pla) continue;
        const libs = getNumLibertiesCapped(stones, adj, 20);
        if (libs > 1) numLibsX2 += libs * 2 - 3;
    }
    return numLibsX2;
}
function getBoundNumLibertiesAfterPlay(stones, loc, pla) {
    const opp = opponentOf(pla);
...
function getNumLibertiesAfterPlay(stones, loc, pla, max) {
    if (stones[loc] !== EMPTY) return 0;
    const opp = opponentOf(pla);
    ladderCapturedStamp++;
...
function findLibertyGainingCaptures(stones, loc, buf, bufStart) {
    const pla = stones[loc];
...
function undoMoveRaw(pos, move, player, koPointBefore, captureStart, captureStack) {
    const captureEnd = captureStack.length;
    const opp = opponentOf(player);
...
function tryPlayMoveNoThrow(pos, move, player, captureStack, koPointBeforeOut, captureStartOut, recordIdx) {
    const koPointBefore = pos.koPoint;
    const captureStart = captureStack.length;
    ...
    if (pos.stones[move] !== EMPTY) return false;
...
function searchIsLadderCaptured(pos, loc, defenderFirst, scratch) {
    if (loc < 0 || loc >= BOARD_AREA) return false;
    const c = pos.stones[loc];
    ...
    while(true){
        ...
        if (searchNodeCount >= LADDER_SEARCH_NODE_BUDGET) {
            for(let i = stackIdx - 1; i >= 0; i--){
                undoMoveRaw(pos, recordMoves[i], recordPlayers[i], recordKoPointBefore[i], recordCaptureStart[i], captureStack);
            }
            pos.koPoint = koSaved;
            return false;
        }
        ...
        if (moveListCur[stackIdx] === -1) {
            ...
            if (returnedFromDeeper) {
                undoMoveRaw(pos, recordMoves[stackIdx], recordPlayers[stackIdx], recordKoPointBefore[stackIdx], recordCaptureStart[stackIdx], captureStack);
            }
...
function searchIsLadderCapturedAttackerFirst2Libs(pos, loc, scratch, outWorkingMoves) {
    if (loc < 0 || loc >= BOARD_AREA) return false;
    const c = pos.stones[loc];
    if (c !== BLACK && c !== WHITE) return false;
...
let LADDER_FEATURES_SCRATCH_V7 = {
    copyPos: {
        stones: new Uint8Array(BOARD_AREA),
        koPoint: -1
    },

import { BOARD_AREA, BOARD_SIZE } from './fastBoard.js';
...
let expectedSVTable = null;
function whiteScoreValueOfScoreSmoothNoDrawAdjust(finalWhiteMinusBlackScore, center, scale, sqrtBoardArea) {
    const adjustedScore = finalWhiteMinusBlackScore - center;
    return Math.atan(adjustedScore / (scale * sqrtBoardArea)) * TWO_OVER_PI;
}
function initScoreValueTables() {
    ...
    const minSVSteps = -(svTableMeanRadius * stepsPerUnit + stepsPerUnit / 2 + boundStdevs * svTableStdevLen * stepsPerUnit);
...
export function getScoreStdev(scoreMean, scoreMeanSq) {
    const variance = scoreMeanSq - scoreMean * scoreMean;
    if (variance <= 0) return 0;
    return Math.sqrt(variance);
}
export const getSqrtBoardArea = ()=>Math.sqrt(BOARD_AREA);
